...
class TransactionVerifierAgent:
    ...
    def run(self):
        ...
        if "ml_score" not in ml_df.columns:
            num = ml_df.drop(columns=["idx"]).select_dtypes(include=[np.number])
            ml_df["ml_score"] = num.mean(axis=1)

        df = pd.merge(ring_df, ml_df, on="idx", how="inner")

        # simple rules
        df["status"] = "pass"
        df.loc[df["ml_score"] >= self.cfg.ml_score_deny, "status"] = "deny"
        df.loc[df["ml_score"].between(self.cfg.ml_score_flag, self.cfg.ml_score_deny), "status"] = "flag"

        # explanation
        df["explanation"] = df.apply(lambda r: f"Transaction {r['idx']} has ml_score={r['ml_score']:.2f}, status={r['status']}", axis=1)
        ...
        report = {
            ...
            "data": df[["idx","status","ml_score","explanation"]].to_dict(orient="records")
        }
